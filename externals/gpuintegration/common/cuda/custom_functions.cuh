...
/*
ES: get max values of type T
*/
#include <cfloat>
template<typename T>
__device__ __host__ T get_max_val();
template<>
__device__ __host__ inline float get_max_val<float>() { return FLT_MAX; }
template<>
__device__ __host__ inline double get_max_val<double>() { return DBL_MAX; }
/*
END
*/
...
blockReduceMinMax(T& min, T& max)
{
  ...
  min = (threadIdx.x < (blockDim.x >> 5)) ? shared_min[lane] : get_max_val<T>();
...
block0_min_max(T* mins, T* maxs, const int size, T* min, T* max)
{
  ...
  T localMin = tid < size ? mins[tid] : get_max_val<T>();
