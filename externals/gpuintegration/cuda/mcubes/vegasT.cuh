...
#define RNMX (1.0 - EPS)
  ...
  vegas(IntegT const& integrand,
        ...
        cudaStream_t stream = 0) // ES Adding cuda stream support.
  {
    ...
    IntegT* d_integrand;
    cudaMallocAsync((void**)&d_integrand, sizeof(IntegT), stream);
    cudaCheckError();
    cudaMemcpyAsync(d_integrand, &integrand, sizeof(IntegT), cudaMemcpyHostToDevice, stream);// ES Adding cuda stream support
    cudaCheckError();
    ...
    nd = ndmx;
    ng = 1;
    ng = (size_t)pow(ncall / 2.0 + 0.25,
                     1.0 / ndim); // why do we add .25?
                                  // std::cout<<"couted ng:"<<pow(ncall / 2.0 +
                                  // 0.25, 1.0 / ndim)<<std::endl;
    ...
    cudaMallocAsync((void**)&result_dev, sizeof(double) * 2, stream); // ES Adding cuda stream support.
    cudaCheckError();
    cudaMallocAsync((void**)&d_dev, sizeof(double) * (ndmx_p1) * (mxdim_p1), stream); // ES Adding cuda stream support.
    cudaCheckError();
    cudaMallocAsync((void**)&dx_dev, sizeof(double) * (mxdim_p1), stream); // ES Adding cuda stream support.
    cudaCheckError();
    cudaMallocAsync((void**)&x_dev, sizeof(double) * (mxdim_p1), stream); // ES Adding cuda stream support.
    cudaCheckError();
    cudaMallocAsync((void**)&xi_dev, sizeof(double) * (mxdim_p1) * (ndmx_p1), stream); // ES Adding cuda stream support.
    cudaCheckError();
    cudaMallocAsync((void**)&regn_dev, sizeof(double) * ((ndim * 2) + 1), stream); // ES Adding cuda stream support.
    cudaCheckError();
    cudaMallocAsync((void**)&ia_dev, sizeof(int) * (mxdim_p1), stream); // ES Adding cuda stream support.
    cudaCheckError();

    cudaMemcpyAsync(dx_dev, dx, sizeof(double) * (mxdim_p1), cudaMemcpyHostToDevice, stream);//cudaMemcpy(dx_dev, dx, sizeof(double) * (mxdim_p1), cudaMemcpyHostToDevice); ES Adding cuda stream support
    cudaCheckError();
    cudaMemcpyAsync(x_dev, x, sizeof(double) * (mxdim_p1), cudaMemcpyHostToDevice, stream);//cudaMemcpy(x_dev, x, sizeof(double) * (mxdim_p1), cudaMemcpyHostToDevice); ES Adding cuda stream support
    cudaCheckError();
    cudaMemcpyAsync(regn_dev,
               regn,
               sizeof(double) * ((ndim * 2) + 1),
               cudaMemcpyHostToDevice, stream); // ES Adding cuda stream support.
    // cudaMemcpy(regn_dev,
    //            regn,
    //            sizeof(double) * ((ndim * 2) + 1),
    //            cudaMemcpyHostToDevice);
    cudaCheckError();
    cudaMemsetAsync(ia_dev, 0, sizeof(int) * (mxdim_p1), stream); //ES Adding cuda stream support
    ...
    for (it = 1; it <= itmax && (*status) == 1; (*iters)++, it++) {
      ...
      cudaMemcpyAsync(xi_dev,
                 xi,
                 sizeof(double) * (mxdim_p1) * (ndmx_p1),
                 cudaMemcpyHostToDevice, stream); // ES Adding cuda stream support 
      cudaCheckError(); // bin bounds
      cudaMemsetAsync(
        d_dev, 0, sizeof(double) * (ndmx_p1) * (mxdim_p1), stream); // bin contributions // ES Adding cuda stream support 
      cudaMemsetAsync(result_dev, 0, 2 * sizeof(double), stream);// ES Adding cuda stream support 
      ...
      vegas_kernel<IntegT, ndim, DEBUG_MCUBES, GeneratorType>
        <<<params.nBlocks, params.nThreads, 0, stream>>>(d_integrand, //ES Adding cuda stream support.
                                              ng,
                                              npg,
                                              xjac,
                                              dxg,
      ...
      //cudaStreamSynchronize(stream);//cudaDeviceSynchronize(); ES Adding cuda stream support.
      cudaMemcpyAsync(xi,
                 xi_dev,
                 sizeof(double) * (mxdim_p1) * (ndmx_p1),
                 cudaMemcpyDeviceToHost, stream); // ES Adding cuda stream support 
      cudaCheckError();
      ...
      cudaMemcpyAsync(
        result, result_dev, sizeof(double) * 2, cudaMemcpyDeviceToHost, stream); // ES Adding cuda stream support 
    ...
    cudaFreeAsync(dx_dev, stream);
    cudaFreeAsync(ia_dev, stream);
    cudaFreeAsync(x_dev, stream);
    cudaFreeAsync(xi_dev, stream);
    cudaFreeAsync(regn_dev, stream);
    cudaFreeAsync(result_dev, stream);
    cudaFreeAsync(d_integrand, stream);
  }
            ...
            cudaStream_t stream = 0) // ES Adding cuda stream support
  {
    ...
    vegas<IntegT, NDIM, DEBUG_MCUBES, GeneratorType>(ig,
                                                     ...
                                                     skipIters,
                                                     volume,
                                                     stream); // ES Adding cuda stream support.
    return result;
  }
